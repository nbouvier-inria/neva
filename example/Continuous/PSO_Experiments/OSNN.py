...
f = lambda x: -cigar(x, x0)                           # Function to maximise
name = "Cigar"
figure = []                                          # phase, maxf, maxfpso, yoft

start = np.array([(np.random.random(D)-0.5)*4 for i in range(N)])     # Starting points for the swarm
delta = 1.1                                                 # Damping factor
teta = 95.2 * (2 * pi) / 360                                  # Rotation in radians
w1 = 10
w2 = 1

num_steps = 360                                             # Number of steps to run the swarm for
...
class PyOSNModel(PyLoihiProcessModel):
    ...
    t: np.ndarray = LavaPyType(np.ndarray, float)
    ...
    def run_spk(self):
        ...
        if abs(self.y) > self.Th:
            ...
            self.y = self.y + cos(teta*self.t) * self.v 
            self.v = delta * self.v + sin(teta*self.t) * self.Th / 100
            self.s_out.send(np.array([False]))
            self.t += 1
...
if __name__ == "__main__":
    ...
    if "phase" in figure:
        ...
        plt.xlabel("Deviation speed v")
        plt.tight_layout()
        plt.savefig(f"Experimental_OSNN_v|y_{name}_N={N}_D={D}")
        plt.close()
    if "yoft" in figure:
        ...
        plt.xlabel("Time t")
        plt.tight_layout()
        plt.savefig(f"Experimental_OSNN_t|y_{name}_N={N}_D={D}")
        plt.close()
    if "maxf" in figure:
        plt.figure(figsize=(10,8))
        plt.plot(r)
        ...
        plt.tight_layout()
        plt.savefig(f"Experimental_OSNN_Max(X)|E(F(X))_{name}_N={N}_D={D}")
        plt.close()
    if "maxfpso" in figure:
        ...
        plt.ylabel("Max(f(X)) / E(f(X))")
        plt.tight_layout()
        plt.savefig(f"PSO_Max(X)|E(F(X))_{name}_N={N}_D={D}")
        plt.close()
